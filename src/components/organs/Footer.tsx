...
const Footer = () => {
    ...
    const arrayTrans = checkLange() === "en" ?  FooterTexts.quickLinks.links :  FooterTexts.quickLinks.linksVi
    return (
        <footer className="w-full bg-zinc-950 flex flex-col">
            <main className="w-full lg:pt-28 lg:pb-12 pt-20 pb-12 px-6 grid md:grid-cols-3 lg:gap-8 md:gap-5 gap-8 lg:px-32">
                <div className="flex flex-col gap-2">
                    ...
                    <Text as="p" className="text-zinc-400 text-justify">{TranText('FooterTexts.underLogoText')}</Text>
                </div>

                {/* Quick Links  */}
                <div className="flex flex-col md:items-center md:mt-8 gap-4">
                    <Text as="h1" className="text-zinc-300 text-2xl font-bold">{TranText('FooterTexts.quickLinks.caption')}</Text>
                    <ul className="flex flex-col gap-2">
                        {
                            arrayTrans.map((link, index) => (
                ...
                <div className="flex flex-col md:mt-8 gap-6">
                    <Text as="h1" className="text-zinc-300 text-2xl font-bold">{TranText('FooterTexts.contacts.caption')}</Text>
            ...
            <div className="text-center py-3 bg-gradient-to-r from-red-500 to-amber-500">
                <Text as="p" className="text-zinc-200 md:text-sm text-xs font-bold">{TranText('FooterTexts.copyright')}</Text>
            </div>
        </footer>
    )
}
